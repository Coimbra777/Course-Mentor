SHELL = /bin/bash
DOCKER_MONGODB=docker exec -it mongodb mongosh -u $(ADMIN_USER) -p $(ADMIN_PASSWORD) --authenticationDatabase admin
DOCKER_MONGODB_WITH_CUSTOM_CREDS=docker exec -it mongodb mongosh -u $(DB_USER) -p $(DB_PASS) --authenticationDatabase $(DB_NAME)
...
mongodb-console: export DB_USER=root
mongodb-console: export DB_PASS=password
mongodb-console: export DB_NAME=mentor_stats
mongodb-console:
